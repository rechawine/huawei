...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn_train,
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for t in range(K):
                ...
                if t != K-1:
                    ...
                    pgd.restore_grad()
                loss_sum, _, _, _ = run_batch_fn_train(args, model, batch)
                
                if args.n_gpu > 1:
                    loss_sum = loss_sum.mean()  # mean() to average on multi-gpu parallel training
                if args.gradient_accumulation_steps > 1:
                    loss_sum = loss_sum / args.gradient_accumulation_steps

                if args.fp16:
                    with amp.scale_loss(loss_sum, optimizer) as scaled_loss_sum:
                        scaled_loss_sum.backward()
                else:
                    loss_sum.backward()
            pgd.restore()              

            # fgm.attack() ## 添加对抗扰动
            # loss_adv, _, _, _ = run_batch_fn_train(args, model, batch)
            # if args.n_gpu > 1:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # epoch_iterator.set_postfix(Loss=tr_loss_adv / local_steps)
        ...
        if args.local_rank in [-1, 0]:
            ...
            # tb_writer.add_scalar("loss", tr_loss_adv / local_steps, global_step)
    ...
    # return global_step, tr_loss_adv / local_steps
